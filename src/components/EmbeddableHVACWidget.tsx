...
interface EmbeddableHVACWidgetProps {
  config?: {
    mode?: ChatMode;
    theme?: 'light' | 'dark';
    primaryColor?: string;
...
const EmbeddableHVACWidget: React.FC<EmbeddableHVACWidgetProps> = ({ config = {} }) => {
  ...
  const toggleChat = () => {
    ...
    setIsOpen(newIsOpen);
    if (!newIsOpen) {
      setCurrentMode(config.mode || 'welcome');
    }
    ...
    <>
      <div className="fixed bottom-4 sm:bottom-6 right-4 sm:right-6 z-40">
        <Button
          ...
          size="icon"
          style={{
            backgroundColor: config.primaryColor || undefined,
            backgroundImage: config.primaryColor ? `linear-gradient(to right, ${config.primaryColor}, ${config.primaryColor}dd)` : undefined
          }}
        >
          {isOpen ? (
      ...
      {isOpen && (
        <div className="fixed inset-x-4 bottom-20 sm:bottom-24 sm:right-6 sm:left-auto z-50 sm:w-96 sm:max-w-[calc(100vw-3rem)]">
          <Card className="shadow-2xl border-2 border-blue-200 rounded-2xl overflow-hidden animate-fade-in bg-white w-full">
            <div 
              className="bg-gradient-to-r from-[#007BFF] to-blue-600 text-white p-4 sm:p-5 flex items-center justify-between"
              style={{
                backgroundColor: config.primaryColor || undefined,
                backgroundImage: config.primaryColor ? `linear-gradient(to right, ${config.primaryColor}, ${config.primaryColor}dd)` : undefined
              }}
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs sm:text-sm text-blue-100 flex items-center">
                      ...
                      {config.subtitle || 'Clobol assistent'}
                    </p>
                  </div>
                </div>
                <Button
...
export default EmbeddableHVACWidget;